...
func (h create) Create(w http.ResponseWriter, r *http.Request, _ common.QueryMap) {
	...
	z, err := h.create.Do(r.Context(), toDomainModel(m, userID))
	if err != nil {
		util.HandleError(w, err)
		return
	}
	...
	// 非リソース所有者の場合
	if userID != z.UserId {
		e := util.Wrap(derr.Unauthorized, fmt.Sprintf("difference userID. request: %d, resource: %d", userID, z.UserId))
		util.HandleError(w, e)
		return
	}
...
func toDomainModel(req PostRequest, userID int) zo.Zo {
	return zo.NewZo(0, req.AchievementDate, req.Exp, req.CategoryId, req.Message, time.Now(), sql.NullTime{}, userID)
}
